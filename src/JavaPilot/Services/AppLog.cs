...
public sealed class AppLog : IDisposable
{
    ...
    public AppLog(string? directory = null)
    {
        ...
        FilePath = Path.Combine(
            directory,
            $"java-pilot-{DateTime.Now:yyyyMMdd-HHmmss-fff}-{Guid.NewGuid():N}.log");
        ...
        Info(
            "SESSION",
            $"启动会话：OS={Environment.OSVersion}; .NET={Environment.Version}; " +
            $"64位系统={Environment.Is64BitOperatingSystem}; 进程={Environment.ProcessPath}");
    }
